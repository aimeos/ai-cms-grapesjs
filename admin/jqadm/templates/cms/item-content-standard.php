				...
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-text-group-item-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="card-tools-right">
							...
							<div v-if="item['cms.lists.siteid'] == siteid"
					...
					<div v-bind:id="'item-text-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<input type="hidden" v-model="item['text.id']"
							v-bind:name="'<?= $enc->attr( $this->formparam( array( 'content', '_idx_', 'text.id' ) ) ); ?>'.replace('_idx_', idx)" />
						...
						<div class="col-xl-6">
							...
							<div class="form-group row mandatory">
								...
								<div class="col-sm-8">
									<select class="form-control form-select item-status" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'content', '_idx_', 'text.status' ) ) ); ?>'.replace('_idx_', idx)"
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-control form-select item-languageid" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
											...
											v-bind:name="'<?= $enc->attr( $this->formparam( ['content', '_idx_', 'text.languageid'] ) ); ?>'.replace('_idx_', idx)"
							...
							<?php else : ?>
								<input class="text-langid" type="hidden"
									v-bind:name="'<?= $enc->attr( $this->formparam( array( 'content', '_idx_', 'text.languageid' ) ) ); ?>'.replace('_idx_', idx)"
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-control form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
											...
											v-bind:name="'<?= $enc->attr( $this->formparam( ['content', '_idx_', 'cms.lists.type'] ) ); ?>'.replace('_idx_', idx)"
											v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'content', '_idx_', 'cms.lists.datestart' ) ) ); ?>'.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'content', '_idx_', 'cms.lists.dateend' ) ) ); ?>'.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:disabled="item['cms.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
						...
						<div v-show="item['_ext']" class="col-xl-6 secondary" v-bind:class="{readonly: item['cms.lists.siteid'] != siteid}">
							<config-table v-bind:tabindex="<?= $this->get( 'tabindex' ); ?>"
								v-bind:keys="JSON.parse('<?= $enc->attr( $this->config( 'admin/jqadm/cms/item/content/config/suggest', [] ) ) ?>')"
								v-bind:name="'<?= $enc->attr( $this->formparam( ['content', '_idx_', 'config', '_pos_', '_key_'] ) ); ?>'"
