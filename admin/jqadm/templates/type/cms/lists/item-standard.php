...
<form class="item item-cms-lists-type form-horizontal" method="POST" enctype="multipart/form-data" action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ); ?>">
	...
	<nav class="main-navbar">
		<h1 class="navbar-brand">
			<span class="navbar-title"><?= $enc->html( $this->translate( 'admin', 'CMS Lists Type' ) ); ?></span>
			<span class="navbar-id"><?= $enc->html( $this->get( 'itemData/cms.lists.type.id' ) ); ?></span>
			<span class="navbar-label"><?= $enc->html( $this->get( 'itemData/cms.lists.type.label' ) ?: $this->translate( 'admin', 'New' ) ); ?></span>
			<span class="navbar-site"><?= $enc->html( $this->site()->match( $this->get( 'itemData/cms.lists.type.siteid' ) ) ); ?></span>
		</h1>
			...
			<div class="navbar-content">
				...
				<div class="item-meta text-muted">
					<small>
						...
						<span class="meta-value"><?= $enc->html( $this->get( 'itemData/cms.lists.type.mtime' ) ); ?></span>
					</small>
					<small>
						<?= $enc->html( $this->translate( 'admin', 'Created' ) ); ?>:
						<span class="meta-value"><?= $enc->html( $this->get( 'itemData/cms.lists.type.ctime' ) ); ?></span>
					</small>
					<small>
						<?= $enc->html( $this->translate( 'admin', 'Editor' ) ); ?>:
						<span class="meta-value"><?= $enc->html( $this->get( 'itemData/cms.lists.type.editor' ) ); ?></span>
					</small>
				</div>
			</div>
		</div>
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<select class="form-control form-select item-domain" required="required" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'cms.lists.type.domain' ) ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/cms.lists.type.siteid' ) ); ?> >
								<option value="">
									<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>
								</option>

								<?php foreach( ['media', 'text'] as $domain ) : ?>
									<option value="<?= $enc->attr( $domain ); ?>" <?= $selected( $this->get( 'itemData/cms.lists.type.domain', 'text' ), $domain ); ?> >
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<select class="form-control form-select item-status" required="required" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'cms.lists.type.status' ) ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/cms.lists.type.siteid' ) ); ?> >
								<option value="">
									<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>
								</option>
								<option value="1" <?= $selected( $this->get( 'itemData/cms.lists.type.status', 1 ), 1 ); ?> >
									<?= $enc->html( $this->translate( 'mshop/code', 'status:1' ) ); ?>
								</option>
								<option value="0" <?= $selected( $this->get( 'itemData/cms.lists.type.status', 1 ), 0 ); ?> >
									<?= $enc->html( $this->translate( 'mshop/code', 'status:0' ) ); ?>
								</option>
								<option value="-1" <?= $selected( $this->get( 'itemData/cms.lists.type.status', 1 ), -1 ); ?> >
									<?= $enc->html( $this->translate( 'mshop/code', 'status:-1' ) ); ?>
								</option>
								<option value="-2" <?= $selected( $this->get( 'itemData/cms.lists.type.status', 1 ), -2 ); ?> >
