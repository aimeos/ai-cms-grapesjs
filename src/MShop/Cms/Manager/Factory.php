...
/**
 * Factory for the cms domain manager
 *
 * @package MShop
 * @subpackage Cms
 */
	...
	implements \Aimeos\MShop\Common\Factory\Iface
{
	...
	public static function create( \Aimeos\MShop\ContextIface $context, string $name = null ) : \Aimeos\MShop\Common\Manager\Iface
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->config()->get( 'mshop/cms/manager/name', 'Standard' );
		}

		$iface = \Aimeos\MShop\Cms\Manager\Iface::class;
		$classname = '\Aimeos\MShop\Cms\Manager\\' . $name;

		if( ctype_alnum( $name ) === false )
		{
			$msg = $context->i18n()->dt( 'mshop', 'Invalid characters in class name "%1$s"' );
			throw new \Aimeos\MShop\Cms\Exception( sprintf( $msg, $classname ) );
		}
