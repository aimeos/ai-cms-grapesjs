...
<div id="content" class="item-content tab-pane fade" role="tablist" aria-labelledby="content">
	...
	<div id="item-content-group"
		data-data="<?= $enc->attr( $this->get( 'contentData', [] ) ) ?>"
		data-images="<?= $enc->attr( $this->get( 'contentMedia', [] ) ) ?>"
					...
					<div class="group-item card" v-bind:class="{mismatch: !can('match', index)}">
						...
						<div v-bind:id="'item-text-group-data-' + index" v-bind:class="element['_show'] ? 'show' : 'collapsed'"
							...
							<input type="hidden" v-model="element['text.id']"
								v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'text.id' ) ) ) ?>`.replace('_idx_', index)">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select class="form-select item-status" required="required" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'text.status' ) ) ) ?>`.replace('_idx_', index)"
							...
							<div class="col-xl-6">
									...
									<div class="form-group row mandatory">
										...
										<div class="col-sm-8">
											<select is="vue:select-component" required class="form-select item-languageid" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
												...
												v-bind:name="`<?= $enc->js( $this->formparam( ['content', '_idx_', 'text.languageid'] ) ) ?>`.replace('_idx_', index)"
								...
								<?php else : ?>
									<input class="text-langid" type="hidden"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'text.languageid' ) ) ) ?>`.replace('_idx_', index)"
							...
							<div class="col-xl-12">
								<grapesjs tabindex="<?= $this->get( 'tabindex' ) ?>"
									...
									v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'text.content' ) ) ) ?>`.replace('_idx_', index)"
									v-bind:readonly="!can('change', index)"
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
									...
									<div class="form-group row mandatory">
										...
										<div class="col-sm-8">
											<select is="vue:select-component" required class="form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
												...
												v-bind:name="`<?= $enc->js( $this->formparam( ['content', '_idx_', 'cms.lists.type'] ) ) ?>`.replace('_idx_', index)"
												v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
												v-bind:readonly="!can('change', index)"
								...
								<?php else : ?>
									<input class="listitem-type" type="hidden"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'cms.lists.type' ) ) ) ?>`.replace('_idx_', index)"
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input is="vue:flat-pickr" class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'cms.lists.datestart' ) ) ) ?>`.replace('_idx_', index)"
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input is="vue:flat-pickr" class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'content', '_idx_', 'cms.lists.dateend' ) ) ) ?>`.replace('_idx_', index)"
											placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
											v-bind:disabled="!can('change', index)"
											v-bind:config="Aimeos.flatpickr.datetime"
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
								<config-table v-bind:tabindex="`<?= $enc->js( $this->get( 'tabindex' ) ) ?>`"
									...
									v-bind:name="`<?= $enc->js( $this->formparam( ['content', '_idx_', 'config', '_pos_', '_key_'] ) ) ?>`.replace('_idx_', index)"
									v-bind:index="index"
									v-bind:readonly="!can('change', index)"
