...
<div id="seo" class="item-seo tab-pane fade" role="tablist" aria-labelledby="seo">
	...
	<div id="item-text-group"
		...
		<div class="group-list" role="tablist" aria-multiselectable="true">
			<div is="vue:draggable" item-key="text.id" group="seo" :list="items" handle=".act-move">
				<template #item="{element, index}">

					<div class="group-item card box" v-bind:class="{loading: element['loading'], readonly: !can('change', index)}">
								...
								<div class="dropdown">
									...
									<div class="dropdown-menu dropdown-menu-end" v-bind:aria-labelledby="'translate-menu-' + index">
										<?php foreach( ['de', 'en', 'es', 'fr', 'it', 'nl', 'pl', 'pt', 'ru'] as $lang ) : ?>
											<a class="dropdown-item" href="#" v-on:click="translate(index, `<?= strtoupper( $lang ) ?>`)"><?= $enc->html( $this->translate( 'language', $lang ) ) ?></a>
										<?php endforeach ?>
									</div>
								</div>
						...
						<div v-bind:id="'item-text-group-data-' + index" v-bind:class="element['_show'] ? 'show' : 'collapsed'"
							...
							<input type="hidden" v-model="element['text.id']"
								v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'text.id' ) ) ) ?>`.replace('_idx_', index)">

							<div class="col-xl-6">

								<div class="form-group row mandatory">
									<div class="col-sm-12">
										<textarea is="vue:html-editor" class="form-control item-content" required="required"
											tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
											v-bind:key="index"
											v-bind:id="'cke-' + index"
											v-bind:editor="CKEditor"
											v-bind:config="Aimeos.ckeditor"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'text.content' ) ) ) ?>`.replace('_idx_', index)"
							...
							<div class="col-xl-6">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select class="form-select item-status" required="required" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'text.status' ) ) ) ?>`.replace('_idx_', index)"
											v-bind:readonly="!can('change', index)"
									...
									<div class="form-group row mandatory">
										...
										<div class="col-sm-8">
											<select is="vue:select-component" required class="form-select item-languageid" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
												...
												v-bind:name="`<?= $enc->js( $this->formparam( ['seo', '_idx_', 'text.languageid'] ) ) ?>`.replace('_idx_', index)"
								...
								<?php else : ?>
									<input class="text-langid" type="hidden"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'text.languageid' ) ) ) ?>`.replace('_idx_', index)"
									...
									<div class="form-group row mandatory">
										...
										<div class="col-sm-8">
											<select is="vue:select-component" required class="form-select item-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
												...
												v-bind:name="`<?= $enc->js( $this->formparam( ['seo', '_idx_', 'text.type'] ) ) ?>`.replace('_idx_', index)"
												v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
												v-bind:readonly="!can('change', index)"
								...
								<?php else : ?>
									<input class="item-type" type="hidden"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'text.type' ) ) ) ?>`.replace('_idx_', index)"
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input class="form-control item-label" type="text" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'text.label' ) ) ) ?>`.replace('_idx_', index)"
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
								...
								<?php if( !( $listTypes = $this->get( 'seoListTypes', map() ) )->count() !== 1 ) : ?>
									<div class="form-group row mandatory">
										...
										<div class="col-sm-8">
											<select is="vue:select-component" required class="form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
												...
												v-bind:name="`<?= $enc->js( $this->formparam( ['seo', '_idx_', 'cms.lists.type'] ) ) ?>`.replace('_idx_', index)"
												v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
												v-bind:readonly="!can('change', index)"
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input is="vue:flat-pickr" class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'cms.lists.datestart' ) ) ) ?>`.replace('_idx_', index)"
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input is="vue:flat-pickr" class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'seo', '_idx_', 'cms.lists.dateend' ) ) ) ?>`.replace('_idx_', index)"
											placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
											v-bind:disabled="!can('change', index)"
											v-bind:config="Aimeos.flatpickr.datetime"
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
								<config-table tabindex="<?= $enc->js( $this->get( 'tabindex' ) ) ?>"
